...
public class MainActivity extends AppCompatActivity {
    FirebaseAutoMLRemoteModel remoteModel; // permite cargar el modelo remotamente
    FirebaseVisionImageLabeler labeler; // permite cargar la imagen con la etiquetadora
    FirebaseVisionOnDeviceAutoMLImageLabelerOptions.Builder optionsBuilder; // permite escojer si la etiquetadora se ejecuta de forma remota o local
    ProgressDialog progressDialog; //muestra el dialogo de carga del modelo
    FirebaseModelDownloadConditions conditions; //condiciones de descarga del modelo
    FirebaseVisionImage image; // imagen de entrada
    TextView textView; // visualizacion de la clasificacion de la imagen de entrada
    Button button; // para seleccionar la imagen
    ImageView imageView; //para desplegar la imegen seleccionada
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                if (result != null) {
                    Uri uri = result.getUri(); //path de la imagen en el telefono
                    imageView.setImageURI(uri); //establer la imagen
                    textView.setText(""); //permite limpiar el texto
...
//                startActivity(intent);
            ...
            public void onFailure(@NonNull Exception e) {
                Log.e("OnFail", "" + e);
                Toast.makeText(MainActivity.this, "Algo salio mal! " + e, Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void fromRemoteModel() {
        progressDialog.show();                                         /* nombre del modelo*/
        ...
        //decargamos el modelo
        FirebaseModelManager.getInstance().download(remoteModel, conditions)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        CropImage.activity().start(MainActivity.this); // recorte de la imagen
                    }
                });
    }
}